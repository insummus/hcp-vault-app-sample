...
sudo echo "export GRADLE_HOME=/opt/gradle/gradle-${GRADLE_VERSION}" | sudo tee /etc/profile.d/gradle.sh
sudo echo "export PATH=\$PATH:\$GRADLE_HOME/bin" | sudo tee -a /etc/profile.d/gradle.sh
...
## 설정 파일 업데이트
- 수정: src/main/resources/application.properties (각 항목을 실제 값으로 변경)
```
vault {
...
# 이후 build/libs에 생성된 JAR 파일을 직접 실행할 수도 있습니다.
